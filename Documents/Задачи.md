...
- Спроектировать схему БД: `violations`, `violation_photos`, `violation_messages`, `violation_helpers`, `device_tokens` — 3 дня
- Реализовать API создания нарушения (multipart photos[]; YOS upload) — 3 дня
- Реализовать чат и быстрые действия (confirm/participate/subscribe/report) — 4 дня
- Реализовать resolve/partial_resolve (комментарий, фото, helpers[]; awaiting_confirmation/moderation) — 3 дня
...
- Документация и мониторинг (metrics, alerts, SLA) — 1 день
  ...
  - POST /violations/{id}/resolve — отметить устранённым (comment, photos[], helpers[])
  - POST /violations/{id}/partial_resolve — частично устранено
