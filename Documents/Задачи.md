...
- Онбординг никнейма (экран ввода псевдонима, проверка уникальности, POST /me/nickname) — 1 день
- Реализовать интерфейс карты и поток создания нарушения (маркер → форма) — 4 дня
- Форма создания: multipart upload `photos[]`, счётчик `0/MAX_PHOTOS_PER_VIOLATION` — 3 дня
- Реализовать офлайн‑БД и очередь синхронизации — 5 дней
- Добавить push‑уведомления (FCM / APNs) — 2 дня
- Экран карточки нарушения: чат (GET/POST /violations/{id}/messages), быстрые действия (confirm/participate/subscribe/report) — 4 дня
- Форма «Решено/Частично решено» с фото и helpers[] — 3 дня
- Экран «Мои нарушения» и фильтры по статусам на карте — 2 дня
- Экран «Мой отчёт» + открытие публичной ссылки — 2 дня
...
- Endpoint `POST /me/nickname` и валидация уникальности — 1 день
- Спроектировать схему БД: `violations`, `violation_requests`, `violation_request_photos`, `violation_messages`, `violation_helpers`, `device_tokens` — 3 дня
- Реализовать API создания нарушения (multipart photos[]; YOS upload; автоматическое создание заявки `open`) — 3 дня
- Реализовать чат и быстрые действия (confirm/participate/subscribe/report) — 4 дня
- Реализовать заявки на закрытие: `POST /violations/{id}/close-request` (статусы: `partially_closed`, `closed`; комментарий, фото; автоматическое изменение статуса Violation) — 3 дня
- Публичный отчёт пользователя: `GET /reports/{user_id}` (простая HTML/JSON) — 2 дня
- Push‑подписки: `POST /devices/register` и отправка базовых уведомлений — 2 дня
- Реализовать push‑уведомления и realtime (WebSocket) сервис — 3 дня
- Фоновые воркеры (после MVP): image processing (resize/webp) — 2 дня; ML moderation — 4 дня
...
- Документация и мониторинг (metrics, alerts, SLA) — 1 день
  ...
  - POST /me/nickname — установить уникальный псевдоним (auth)

  - POST /violations — создать нарушение (multipart `photos[]`) (auth)
  - GET /violations — список нарушений (bbox, type, status, date, pagination)
  - GET /violations/{id} — детали (включая фото)
  - GET /violations/{id}/messages — лента сообщений
  - POST /violations/{id}/messages — добавить сообщение
  - POST /violations/{id}/confirm — подтвердить нарушение
  - POST /violations/{id}/participate — участвовать/отказаться
  - POST /violations/{id}/subscribe — подписаться на уведомления
  - POST /violations/{id}/report — жалоба модератору
  ...
  - GET /reports/{user_id} — публичный отчёт пользователя (public)
