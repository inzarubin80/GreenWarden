...
- Статусы: New, Confirmed, Resolved, Partially Resolved. Пользователь может подтверждать и отслеживать статус.
...
- БД: PostgreSQL — таблицы users, violations, violation_requests, violation_request_photos, confirmations, notifications, sessions.
- Ключевые API:
  - POST /auth/oauth/{provider}/callback — обмен кода на токен и выдача JWT
  - GET /me — профиль
  - POST /me/nickname — задать уникальный псевдоним (только для новых пользователей)
  ...
  - GET /violations/{id}/messages — получить сообщения чата
  - POST /violations/{id}/messages — отправить сообщение в чат
  - POST /violations/{id}/confirm — подтвердить
  - POST /violations/{id}/participate — участвовать/отказаться от участия (идемпотентно)
  - POST /violations/{id}/subscribe — подписаться на уведомления (идемпотентно)
  - POST /violations/{id}/report — пожаловаться модератору (текст + файлы)
  - POST /violations/{id}/close-request — создать заявку на закрытие (status: `partially_closed` или `closed`, comment, photos[])
...
- Медиа (MVP): прямой multipart upload на сервер в `POST /violations` (поле `photos[]`), сервер стримит в Yandex Object Storage и возвращает `url` и `thumb_url`. Presigned upload — опционально для v2.
...
- Violation
  ...
  - `status`: текущий статус нарушения (enum: `new`, `confirmed`, `resolved`, `partially_resolved`).
  ...
  - Примечание: фото хранятся в заявках (`violation_requests`), а не напрямую в нарушениях.

- ViolationRequest (заявка)
  - `id`: уникальный идентификатор заявки (UUID).
  - `violation_id`: внешний ключ на нарушение (`Violation.id`).
  - `status`: статус заявки (enum: `open`, `partially_closed`, `closed`).
  - `created_by_user_id`: внешний ключ на автора заявки (`User.id`).
  - `comment`: комментарий при закрытии (NULL для заявки на создание).
  - `photos`: массив фотографий, связанных с заявкой (см. `ViolationRequestPhoto` ниже).
  - `created_at`: время создания заявки (ISO 8601).
  - `updated_at`: время последнего изменения заявки (ISO 8601).
  - Логика:
    - При создании Violation автоматически создается заявка со статусом `open` (created_by_user_id = user_id из Violation).
    - При создании заявки на частичное закрытие (`partially_closed`): статус Violation → `partially_resolved`.
    - При создании заявки на полное закрытие (`closed`): статус Violation → `resolved`.

- ViolationRequestPhoto
  - `id`: уникальный идентификатор фотографии (UUID).
  - `request_id`: внешний ключ на заявку (`ViolationRequest.id`).
  - `url`: публичная ссылка (CDN/публичная политика) на оригинал изображения.
  - `thumb_url`: публичная ссылка на миниатюру (если есть).
  - `created_at`: время загрузки фотографии (ISO 8601).
  - Примечание: все фото хранятся только в этой таблице, привязаны к заявкам, а не напрямую к Violation.

### Создание нарушения — контракт (MVP)

- URL: `POST /violations`
- Auth: `Authorization: Bearer <access_token>`
- Content-Type: `multipart/form-data` (рекомендуется) или `application/json` (без фото)

UX требования:
- Создание происходит после установки пользователем маркера на карте и подтверждения позиции.
- В форме отображать счётчик фотографий: `текущее/MAX_PHOTOS_PER_VIOLATION`.

Multipart (поддерживает несколько фото):
- Поля:
  - `type` (string, required): `garbage|pollution|air|deforestation|other`
  - `description` (string, optional)
  - `lat` (float, required)
  - `lng` (float, required)
  - `photos` (file[], optional): массив изображений (лимит — из `.env`)
- Ответ 200 (JSON):
```json
{
  "id": "uuid",
...
```
Примечание: фото возвращаются через заявки (см. `GET /violations/{id}`).
```

JSON (без фото):
```json
{ "type": "garbage", "description": "...", "lat": 55.75, "lng": 37.62 }
```

Ограничения:
- `MAX_PHOTOS_PER_VIOLATION` (по умолчанию 5) — задаётся в `.env`.
- Базовая валидация: диапазоны координат, допустимые типы, MIME изображений.

### Получение нарушения по ID — контракт

- URL: `GET /api/violations/{id}`
- Auth: не требуется (публичный доступ)
- Content-Type: `application/json`

Параметры:
- `id` (UUID, required) — идентификатор нарушения в пути URL
  ...
  "photos": [
    {
      "id": "uuid",
      "request_id": "uuid",
      "url": "https://cdn/bucket/path.jpg",
      "thumb_url": "https://cdn/bucket/path-thumb.jpg"
    }
  ]
}
```
...
- `404 Not Found` — нарушение с указанным ID не найдено
...
Примечание: endpoint возвращает основные поля для отображения нарушения: ID автора, описание, координаты. Фото загружаются через заявки (все заявки для Violation, включая заявку `open` при создании и заявки на закрытие).

### Чат и быстрые действия
- Для каждого нарушения доступен чат:
  - `GET /violations/{id}/messages` — пагинируемая лента сообщений
  - `POST /violations/{id}/messages` — отправка сообщения (text + опциональные файлы в будущей версии)
- Быстрые действия:
  - `POST /violations/{id}/confirm`
  - `POST /violations/{id}/participate`
  - `POST /violations/{id}/subscribe`
  - `POST /violations/{id}/report` — жалоба модератору (text + файлы)

### Завершение/частичное завершение
- Пользователь или участники могут создать заявку на закрытие:
  - `POST /violations/{id}/close-request` — поля: `status` (required: `partially_closed` или `closed`), `comment` (optional), `photos[]` (optional)
  ...
  - Все фото привязываются к заявке на закрытие (таблица `violation_request_photos`)
  - Может быть несколько заявок на закрытие одного Violation (история всех попыток)

### Отчёт пользователя и публичная ссылка
- `GET /me/report` — агрегированная статистика пользователя (создал, помог решить, подтверждения).
- `POST /me/report/share` — создать публичную ссылку.
- `GET /reports/{user_id}` — публичная страница/JSON c визуализацией.
...
- `YOS_SECRET_KEY`
- `YOS_BUCKET`
- `YOS_ENDPOINT` (например, `storage.yandexcloud.net`)
- `YOS_CDN_BASE_URL` (опционально, если настроен CDN)
- `MAX_PHOTOS_PER_VIOLATION` (например, `5`)

Примечание: в БД можно не использовать жёсткие FK (связи — по идентификаторам), в соответствии с требованиями проекта.

### Дополнительные таблицы (черновик схемы)
- `violation_requests`: `id UUID`, `violation_id UUID`, `status TEXT` (`open`, `partially_closed`, `closed`), `created_by_user_id BIGINT`, `comment TEXT`, `created_at timestamptz`, `updated_at timestamptz`
  - При создании Violation автоматически создается заявка со статусом `open`
  - Заявки на закрытие (`partially_closed`/`closed`) меняют статус Violation
- `violation_request_photos`: `id UUID`, `request_id UUID`, `url TEXT`, `thumb_url TEXT`, `created_at timestamptz`
  - Все фото хранятся только в этой таблице, привязаны к заявкам
- `violation_messages`: `id UUID`, `violation_id UUID`, `user_id bigint`, `text text`, `created_at timestamptz`
- `violation_helpers`: `violation_id UUID`, `user_id bigint`, `added_at timestamptz`
- `device_tokens`: `user_id bigint`, `token text`, `platform text`, `created_at timestamptz`

### Пример: список нарушений

- Запрос:
```
GET /violations?bbox=37.50,55.60,37.80,55.85&type=garbage&status=new&page=1&page_size=20
```
- Параметры:
  - `bbox=minLng,minLat,maxLng,maxLat` — опционально
  - `type` — `garbage|pollution|air|deforestation|other`
  - `status` — `new|confirmed|resolved|partially_resolved`
  - `from`, `to` — ISO8601 (опционально)
...
4) Сохранение как «New» (Store): если отчёт считается новым — создаётся запись в БД с статусом `new`, автоматически создается заявка со статусом `open`, фото привязываются к этой заявке (таблица `violation_request_photos`), и фиксируются временные метки.
...
8) Предложение закрытия (ProposeResolve): автор или другой пользователь может создать заявку на закрытие (`POST /violations/{id}/close-request`), прикрепив новые фото/доказательства; при создании заявки со статусом `partially_closed` или `closed` статус Violation автоматически меняется на `partially_resolved` или `resolved` соответственно.
...
11) Закрытие (ApproveResolve / AutoResolve): при создании заявки на закрытие статус Violation автоматически меняется (не дожидаясь модерации). Система отправляет уведомления владельцу и участникам, логирует событие и обновляет метрики. Все заявки на закрытие сохраняются для истории.
