...
- Ключевые API:
  ...
  - POST /violations/{id}/resolve — отметить устраненным (с фото, списком помощников)
  - POST /violations/{id}/partial_resolve — частично устранено (с фото, списком помощников)
...
- Violation
  ...
  - `photos`: массив фотографий, связанных с нарушением (см. `Photo` ниже).
...
- Photo
  ...
  - `storage_key` (опц., v2/presigned): внутренний ключ/путь в S3.
...
```json
{
  ...
  "photos": [
    { "id": "uuid", "violation_id": "uuid", "url": "https://cdn/bucket/path.jpg", "thumb_url": "https://cdn/bucket/path-thumb.jpg" }
  ],
...
Примечание: endpoint возвращает только основные поля для отображения нарушения: ID автора, описание, координаты и массив фотографий.
...
- Пользователь или участники могут отправить отчёт:
  - `POST /violations/{id}/resolve` — поля: `comment` (required), `photos[]`, `helpers[]: user_id[]`
  - `POST /violations/{id}/partial_resolve` — аналогично
- После отправки может устанавливаться флаг `awaiting_confirmation` или использоваться `moderation_status=pending`.
